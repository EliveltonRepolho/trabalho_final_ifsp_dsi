 ...
 * @author repolho
 */
public class ModeloTabelaMesa extends AbstractTableModel{
    
    private String[] columns = {"Número", "Qtde Lugares", "Status", "#"};
    private List<Mesa> mesas;

    public ModeloTabelaMesa(){
        mesas = new ArrayList<>();
    }
    ...
    public void removeRow(Mesa remove){
        
        List<Mesa> deletados = new ArrayList<>();
        
        for (Mesa f : mesas) {
            if(f.getNumero()== remove.getNumero())
                deletados.add(f);
        }
        
        mesas.removeAll(deletados);
        
        fireTableDataChanged();  
    }
    
    public List<Mesa> getSelecionados(){
        List<Mesa> selecionados = new ArrayList<>();
    ...
    public void setValueAt(Object value, int row, int column) {
        if (column == 3) {
            mesas.get(row).setSelecionado((boolean) value);
